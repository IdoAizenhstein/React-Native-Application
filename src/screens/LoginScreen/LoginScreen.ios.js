...
import {getDeviceLang, getEmoji, getErrText, goTo, isRtl as checkRtl, sp} from '../../utils/func'
import jwtDecode from 'jwt-decode'
import {OVERVIEW, SIGNUP, UPLOADING_DOCUMENTS} from '../../constants/navigation'
...
import {ALERTS_TRIAL, BASE_URL, HOST, HOSTS, IS_DEV, setDevModeFunc, setLocalModeFunc} from '../../constants/config'
...
// smsSubscription = null
    ...
    changeDevMode = async () => {
        if (HOST === HOSTS.dev) {
            setLocalModeFunc()
            Toast.show('Set Local environment', Toast.LONG, Toast.TOP)
        } else {
            setDevModeFunc()
            Toast.show('Set Dev environment', Toast.LONG, Toast.TOP)
        }
    ...
    handleLoginSubmit = () => {
        ...
        if (!username || !password || inProgress || errMail) {
            return
        }
        Keyboard.dismiss()
        this.setState({
    ...
    handleResetPassword = () => {
        ...
        return dispatch(resetPassword(username))
            .then(({tokenInfo}) => {
                this.setState({tokenInfo})
                if (tokenInfo && tokenInfo.maskedPhoneNumber &&
    ...
    handleChangePassword = () => {
        ...
        const {dispatch} = this.props
        if (!password || password !== confirmPassword || inProgress ||
            password.length < 8 || password.length >= 12 ||
            password.replace(/[^\d]/g, '').length === 0 ||
            password.replace(/[^A-Za-z]/g, '').length === 0 || (password.length > 0 &&
                (jwtDecode(this.props.token) && jwtDecode(this.props.token).sub ===
                    password))) {
            return
        }
        Keyboard.dismiss()
            ...
            .then(() => {
                if (this.props.user.changePasswordRequired === 1 ||
                    ...
                    return Promise.all([
                        ...
                        .then(() => {
                            goTo(navigation, this.props.globalParams.ocrPilot !== undefined && (this.props.globalParams.ocrPilot === 1 || this.props.globalParams.ocrPilot === 3) ? UPLOADING_DOCUMENTS : OVERVIEW)
                        })
                        .catch(err => {
                            this.setState({inProgress: false})
                            Toast.show(getErrText(err), Toast.LONG, Toast.BOTTOM)
    ...
    handleTogglePasswordVisibilityDyna = (state) => () => {
        if (state === 'secureTextEntryNew') {
            ...
            this.setState(
                {secureTextEntryRepeat: !this.state.secureTextEntryRepeat})
        }
    }
    handleLinkSignUp = () => {
            ...
            VersionCheck.getLatestVersion({
                ...
                .then(latestVersion => {
                    this.setState({
                        isVersionType:
                            latestVersion ?
                                (VERSION.toString().replace(/\s\s+/g, ' ') === latestVersion.toString().replace(/\s\s+/g, ' '))
                                :
                                false,
                    })
                })
        } else {
            this.setState({isVersionType: true})
        }
                ...
                <View style={[
                            ...
                            <View style={{
                                ...
                                <View style={{
                                        ...
                                        <CheckBox
                                            ...
                                            checkedIcon={<Image style={{
                                                ...
                                                marginTop: 5,
                                                                ...
                                                                source={require(
                                                                    'BiziboxUI/assets/checkboxChecked.png')}/>}
                                    ...
                                    <TouchableOpacity style={styles.linkBtn}
                                        ...
                                        <Text style={styles.linkBtnText}>{t(
                    ...
                    {(screen === SCREEN.OTP_RESET_PASSWORD) && (
                        <Fragment>
                            <View style={{
                                ...
                                <TextInput
                                    placeholder={tokenInfo.maskedPhoneNumber +
                                    'הקלידו את הקוד שנשלח לנייד '}
                                    placeholderTextColor="#202020"
                                    autoCorrect={false}
                                    autoCapitalize="none"
                                    returnKeyType="done"
                    ...
                    {screen === SCREEN.CHANGE_PASSWORD && (
                        <Fragment>
                            <View style={{
                                ...
                                <TextInput
                                    ...
                                    style={[
                                        styles.input,
                                        {
                                            borderBottomColor: ((password.length > 0 &&
                                                    confirmPassword.length > 0 && password !==
                                                    confirmPassword) ||
                                                (password.length > 0 && jwtDecode(this.props.token) &&
                                                    jwtDecode(this.props.token).sub === password))
                                                ? colors.red2
                                                : colors.blue3,
                                            borderBottomWidth: 2,
                                            fontFamily: fonts.semiBold,
                                ...
                                <TextInput
                                    ...
                                    style={[
                                        styles.input,
                                        {
                                            borderBottomColor: ((password.length > 0 &&
                                                confirmPassword.length > 0 && password !==
                                                confirmPassword)) ? colors.red2 : colors.blue3,
                                            borderBottomWidth: 2,
                                            fontFamily: fonts.semiBold,
                                            fontWeight: 'normal',
                                            color: colors.blue29,
                            ...
                            {password.length > 0 && confirmPassword.length > 0 && password !==
                            confirmPassword && (
                                <View>
                                    <Text style={{
                                        fontSize: sp(16),
                    ...
                    {screen === SCREEN.ERROR_401 && (
                        <Fragment>
                            <Text
                                style={[styles.infoText]}>{'נעשו 3 ניסיונות כניסה לא נכונים'}</Text>
                            <Text
                                style={[styles.infoText]}>{'לא ניתן להמשיך בתהליך ההזדהות'}</Text>
